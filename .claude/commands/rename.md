---
description: Propose a better name for a device or entity, then rename it and patch all consumers once approved
---

Help fix poorly-named devices/entities. Input is a device name/id or an `entity_id` (e.g. `light.lr_1`, or a device name like "Living Room Plug 3"). This command both proposes the rename and (once approved) applies it live and patches every consumer — it is not read-only.
