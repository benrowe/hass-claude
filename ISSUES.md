...
- (2026-06-24, `/conventions`) `climate.living_room_air_conditioning`, `climate.man_cave_air_conditioning`, `climate.study_air_conditioning` consistently use `_air_conditioning`, but `CLAUDE.md`'s Common Entity Patterns documents `climate.<area>_ac`. Likely the doc is stale, not the entities — cheap fix is updating `CLAUDE.md`.
